...
# login: 로그인 처리 - 로그인 사용자의 Model을 session에 저장(로그아웃할 때까지 정보 유지)
# logout: 로그아웃 처리 - 로그인 한 사용자 Model을 session에서 제거()
# authenticate: 인증확인 - username(id), password DB에서 확인하는 역할.
# update_session_auth_hash: 회원정보 수정에서 사용. 수정된 정보를 session의 User Model에 적용.
from django.contrib.auth.decorators import login_required

from django.contrib.auth.forms import AuthenticationForm, PasswordChangeForm    # AuthenticationForm: 로그인 폼, PasswordChangeForm: 비밀번호 변경 폼
...
def user_detail(request):
    # View에서 로그인한 사용자 정보를 조회 -> request.user(모델) 사용 
...
## GET: 패스워드 변경 폼을 응답. (응답: password_change.html)
## POST: 패스워드 변경 처리 (응답: /account/detail)
@login_required
def password_change(request):
    http_method = request.method
    if http_method == "GET":
        form = PasswordChangeForm(request.user)    # 비밀번호 변경할 user 정보를 넣어 빈 폼을 생성 - 이전 패스워드 확인용.
        ...
        if form.is_valid():    # 검증 통과
            # db sudate
            user = form.save()    # ModelForm.save(): Update/Insert한 모델 객체
            # session에 저장된 user 정보를 변경된 내용으로 변경. (안하면 logout)
...
def user_update(request):
    ...
    if http_method == "GET":
        ...
        form = CustomUserChnageForm(instance=request.user)
        return render(request, "account/update.html", {"form":form})
    
    elif http_method == "POST":
        # 요청파라미터 조회 + 검증 : Form
        form = CustomUserChnageForm(request.POST, request.FILES, instance=request.user)
