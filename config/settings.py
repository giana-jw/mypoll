...
# 1. app/static 아래 저장 -> static 파일들을 자동으로 장고 서버가 인식
# 2. 1 이외 경로에 static 파일을 저장 -> setting.py에 설정
#############################################################################
# static 파일을 client에서 요청할 때 사용할 url의 시작 path
# account/static/img/join.ipg
...
## app/static, staticfiles_dirs 디렉토리들에 있는 static 파일들을 STATIC_ROOT 디렉토리 아래로 모아준다. (copy)

# - 운영환경
## http 서버 + wsgi (django 실행환경) 나눠서 서버를 구축할 때 http 서버가 static 파일 요청을 처리
